            ...
            <form method="POST" action="{{ route('clientes.pets.update', [$cliente, $pet]) }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $pet->name) }}" required class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-brand/20 focus:border-brand">
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="breed" value="{{ old('breed', $pet->breed) }}" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-brand/20 focus:border-brand">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Peso (kg)</label>
                        <input type="number" step="0.1" name="weight" value="{{ old('weight', $pet->weight) }}" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-brand/20 focus:border-brand">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Alergias</label>
                    <textarea name="allergies" rows="2" class="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-brand/20 focus:border-brand resize-none">{{ old('allergies', $pet->allergies) }}</textarea>
                </div>
                <div>
