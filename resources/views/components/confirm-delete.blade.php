@props(['action', 'itemName' => 'este item'])
...
<span x-data="{ open: false }">
    ...
    <div x-show="open" x-cloak
        ...
        <div x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
             class="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-sm z-10 p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="font-semibold text-gray-800 dark:text-gray-100">Confirmar exclusão</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Tem certeza que deseja excluir {{ $itemName }}? Esta ação não pode ser desfeita.</p>
                </div>
            </div>
