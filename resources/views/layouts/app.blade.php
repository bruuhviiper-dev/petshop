...
<body class="font-sans antialiased bg-gray-100 min-h-screen" x-data="{ sidebarOpen: window.innerWidth >= 1024 }">

    {{-- Sidebar --}}
    <aside class="fixed top-0 left-0 h-full w-60 bg-gray-900 text-white flex flex-col z-30 shadow-xl transition-transform duration-200"
        ...
        <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
            ...
            @foreach($links as $link)
                <a href="{{ route($link['route']) }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs(explode('.', $link['route'])[0].'*') ? 'bg-brand text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
            ...
            <div class="pt-3 mt-3 border-t border-gray-700">
                <a href="{{ route('configuracoes.petshop') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('configuracoes*') ? 'bg-brand text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="transition-all duration-200" :class="sidebarOpen ? 'ml-60' : 'ml-0'">
        ...
        <header class="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-20">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-base font-semibold text-gray-800">{{ $title ?? 'PetAgenda' }}</h1>
            </div>
            <div class="flex items-center gap-3">
                @if(isset($currentPetshop))
                    <a href="{{ route('publico.agendar', $currentPetshop->slug) }}" target="_blank"
                       class="text-xs text-gray-500 hover:text-brand transition-colors flex items-center gap-1">
